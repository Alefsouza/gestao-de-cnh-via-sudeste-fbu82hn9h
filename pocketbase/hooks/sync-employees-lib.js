/**
 * Biblioteca da sincronização "Matriz -> employees".
 *
 * Exporta UMA função (runEmployeesSync) que:
 * - Lê a secret `VW_CONTROLE_CNH` (URL da view externa) via $os.getenv.
 * - Faz fetch na URL e faz upsert em `employees` dedup por `registro` (fallback `chapa`/`cpf`).
 * - Mapeia TODOS os campos, normalizando valores de select.
 * - Espelha a view: quando ela responde dados, remove registros que NÃO vieram nela
 *   (assim os dados seed/mocados somem). Se o fetch falhar ou vier vazio, NADA é apagado.
 * - Grava cada execução em `sync_runs` (Em andamento → Sucesso/Falha) e devolve o registro.
 *
 * O cron (`sync-employees.pb.js`) e a rota manual (`sync-employees-run.js`) chamam esta função.
 */

/**
 * Executa a sincronização completa e retorna o registro de `sync_runs` criado.
 * @param {string} triggerLabel - origem da execução ('cron' | 'manual').
 */
function runEmployeesSync(triggerLabel) {
  ...
  try {
    ...
    $app.save(run)
    console.log(
      'sync:employees (' + triggerLabel + ') — Sucesso:',
    ...
    $app.save(run)
    console.log(
      'sync:employees (' + triggerLabel + ') — Falha:',
