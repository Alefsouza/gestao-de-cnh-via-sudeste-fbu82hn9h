...
export default function CartaProcessoModal({
  ...
  // Sub-estado do formulário de upload dentro de cada colaborador aberto
  const [anexoTitulos, setAnexoTitulos] = useState<Record<string, string>>({})
  ...
  const handleAddPendingAnexo = (
    ...
    const tituloSugerido = anexoTitulos[key] || file.name
    const newAnexo = {
    ...
    setPendingAnexos((prev) => ({
      ...prev,
      [key]: [...(prev[key] || []), newAnexo],
    }))

    // Se estiver em modo edição (já existe carta e processo salvos), faz upload direto
    if (isEditMode && (numeroCarta || colab.processoId)) {
      void uploadSingleAnexoNow(key, file, tituloSugerido, colab)
    }

    toast.success(`Arquivo "${file.name}" adicionado para ${colab.nome}.`)
  }
                  ...
                  >
                    ...
                    {isExpanded && (
                      <div className="border-t p-3.5 space-y-3 bg-muted/10">
                        ...
                        {/* Formulário rápido para selecionar tipo/título e anexar arquivo */}
                        <div className="rounded-md border bg-background/80 p-2.5 space-y-2">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <div className="space-y-1">
                              ...
                              <Select
                                value={anexoTitulos[key] || ''}
                                onValueChange={(val) =>
                                  setAnexoTitulos((prev) => ({ ...prev, [key]: val }))
                                }
                                disabled={saving}
                              >
                                ...
                                <SelectContent>
                                  {TIPOS_ANEXO_SUGERIDOS.map((sugestao) => (
                            ...
                            <div className="flex flex-col justify-end space-y-1">
                              ...
                              <label className="inline-flex h-8 items-center justify-center gap-1.5 cursor-pointer rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
                                <Upload className="h-3.5 w-3.5" />
                                <span>Escolher arquivo e anexar</span>
                                <input
                                  type="file"
                                  className="hidden"
                                  disabled={saving}
                                  ...
                                  onChange={(e) => {
                                    handleAddPendingAnexo(key, e.target.files, colab)
                        ...
                        {/* Lista de anexos já salvos no backend */}
                        {salvos.length > 0 && (
                          <div className="space-y-1.5">
                            <span className="text-[11px] font-semibold text-emerald-800 flex items-center gap-1">
                              <CheckCircle2 className="h-3 w-3" />
                              Arquivos salvos no servidor ({salvos.length}):
                            </span>
                            <div className="space-y-1">
                              {salvos.map((anexo) => {
                                ...
                                return (
                                  <div
                                    key={anexo.id}
                                    className="flex items-center justify-between gap-2 rounded-md bg-white p-2 text-xs border border-border/80"
                                  >
                                    <div className="flex items-center gap-2 min-w-0">
                                      ...
                                      <div className="min-w-0">
                                        ...
                                        <p className="text-[10px] text-muted-foreground truncate">
                                          {anexo.arquivo} {tamanhoKb && `· ${tamanhoKb}`}
                                          {anexo.created &&
                                            ` · Anexado em ${formatDate(anexo.created)}`}
                                        </p>
                                      </div>
                                    </div>
                        ...
                        {/* Lista de anexos pendentes de envio (novos adicionados agora) */}
                        {pendentes.length > 0 && (
                          <div className="space-y-1.5">
                            <span className="text-[11px] font-semibold text-primary flex items-center gap-1">
                              <Upload className="h-3 w-3" />
                              Novos anexos a serem salvos ({pendentes.length}):
                            </span>
                            <div className="space-y-1">
                              {pendentes.map((anexo) => (
                                <div
                                  key={anexo.id}
                                  className="flex items-center justify-between gap-2 rounded-md bg-amber-50/50 border border-amber-200/80 p-2 text-xs"
                                >
                                  <div className="flex items-center gap-2 min-w-0">
                                    <FileText className="h-4 w-4 flex-none text-amber-600" />
                                    <div className="min-w-0">
                                      <p className="font-semibold text-foreground truncate">
                                        {anexo.titulo}
                                      </p>
                                      <p className="text-[10px] text-muted-foreground truncate">
                                        {anexo.file.name} ({(anexo.file.size / 1024).toFixed(0)} KB)
                                      </p>
                                    </div>
                                  </div>
                        ...
                        {totalAnexos === 0 && (
                          <div className="rounded border border-dashed p-3 text-center text-xs text-muted-foreground italic">
                            Nenhum anexo adicionado para este colaborador ainda.
                          </div>
                        )}
                      </div>
                    )}
